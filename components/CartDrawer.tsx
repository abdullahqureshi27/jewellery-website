        ...
        >
          ...
          <SheetHeader className="px-5 py-4 bg-[#0B0D13] border-b border-white/10 flex-shrink-0 pr-12">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-[#CBD5E1]">
                <ShoppingBag className="w-3.5 h-3.5" />
              </div>
              <SheetTitle className="font-serif text-base font-bold tracking-wide text-white">
                Shopping Cart
              </SheetTitle>
              <span className="ml-1 bg-slate-800 text-[#CBD5E1] border border-slate-700 text-[10px] font-bold px-2 py-0.5 rounded-full">
                {totalItems}
              </span>
            </div>
          ...
          >
            {items.length === 0 ? (
              <div className="py-20 text-center space-y-3">
                ...
                <div className="pt-2">
                  <Button
                    ...
                    className="inline-flex items-center gap-1.5 bg-white hover:bg-[#CBD5E1] text-[#0F172A] px-5 py-2 rounded-xl text-xs font-semibold uppercase tracking-wider transition-all duration-300"
                  >
              ...
              items.map(({ product, quantity }) => (
                <div
                  key={product._id}
                  className="bg-[#131720] border border-white/8 hover:border-slate-500/40 rounded-xl p-2.5 flex gap-3 items-center transition-all duration-300 group shadow-xs"
                >
                  {/* Clickable Image -> Sends to Product Detail Page */}
                  <Link
                    href={`/product/${product.slug}`}
                    onClick={() => setIsCartDrawerOpen(false)}
                    className="relative w-16 h-16 rounded-lg overflow-hidden bg-black/60 flex-shrink-0 border border-white/10 group-hover:border-slate-400/50 transition-colors"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-1">
                      <span className="font-mono text-[9px] text-[#CBD5E1] font-semibold tracking-wider">
                        {product.itemCode}
                      </span>
                      <button
                    ...
                    <Link
                      href={`/product/${product.slug}`}
                      onClick={() => setIsCartDrawerOpen(false)}
                      className="font-serif text-xs font-semibold text-white hover:text-[#CBD5E1] transition-colors truncate block leading-snug"
                      title={product.title}
                    >
                      {product.title}
                    </Link>
          ...
          {items.length > 0 && (
            <SheetFooter className="p-4 bg-[#10141D] border-t border-white/10 space-y-2.5 flex flex-col sm:flex-col items-stretch flex-shrink-0 shadow-lg">
              ...
              <div className="flex items-center gap-1.5 text-[10px] text-[#8B949E]">
                <ShieldCheck className="w-3.5 h-3.5 text-[#CBD5E1] flex-shrink-0" />
              ...
              <Button
                ...
                className="w-full h-12 bg-white hover:bg-[#CBD5E1] text-[#0F172A] rounded-xl text-xs font-bold uppercase tracking-[0.15em] transition-all duration-300 shadow-lg shadow-white/5 flex items-center justify-center gap-2.5 cursor-pointer hover:scale-[1.01] active:scale-[0.99] group"
              >
