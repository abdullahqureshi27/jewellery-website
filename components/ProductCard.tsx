...
export default function ProductCard({ product, onQuickView }: ProductCardProps) {
  ...
  return (
    <div className="group relative bg-card rounded-2xl overflow-hidden border border-border luxury-card-shadow flex flex-col justify-between transition-all duration-300">
      ...
      <div className="relative aspect-square w-full bg-muted overflow-hidden">
        ...
        >
          <button
            ...
            className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 shadow-md cursor-pointer ${
              inCart
                ? 'bg-foreground text-background ring-2 ring-border scale-105'
                : 'bg-foreground/85 hover:bg-foreground text-background hover:text-silver-light backdrop-blur-sm'
            }`}
            title={inCart ? 'In your shopping bag (Click to remove)' : 'Add to shopping bag'}
        ...
        {/* 2. CENTER OVERLAY: Quick View Pill Button */}
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center p-4 pointer-events-none">
          <button
            ...
            className="w-[146px] sm:w-[156px] py-2 sm:py-2.5 px-3 rounded-full bg-background text-foreground hover:bg-muted hover:scale-105 active:scale-95 text-xs sm:text-[13px] font-medium tracking-normal text-center shadow-[0_4px_16px_rgba(0,0,0,0.12)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.18)] transition-all duration-300 ease-out transform opacity-0 -translate-y-5 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto cursor-pointer flex items-center justify-center border border-border"
          >
      ...
      <div className="p-4 sm:p-5 flex flex-col justify-between flex-1">
        <div>
          <span className="text-[11px] text-muted-foreground uppercase tracking-widest font-medium mb-1 block">
            {product.category}
          </span>

          <Link href={`/product/${product.slug}`}>
            <h3 className="font-serif text-base font-bold text-foreground hover:text-muted-foreground transition-colors line-clamp-1 mb-1">
              {product.title}
            </h3>
          </Link>
        ...
        <div className="pt-3 border-t border-border flex items-center justify-between gap-2">
          <div className="min-w-0">
            <span className="font-serif text-base sm:text-lg font-bold text-foreground tracking-normal">
              {formattedPrice}
            </span>
          </div>
        </div>
